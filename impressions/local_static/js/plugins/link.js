 ...
 *
 * Customized by Don Button March 2016
 * 1. Hard-wired pop_item as a style. Later might be able to draw it from link options
 * 2. Hijacked the "text" field for short-name entry.
 *   a. changed name attribute to shortnametext
 *   b. entry is appended to url (path)
 */
...
(function (factory) {
  ...
  $.FroalaEditor.PLUGINS.link = function (editor) {
    ...
    function _refreshInsertPopup () {
      ...
      if (link) { // don note: link already exists, show edit pop (not full link insert dialog)
        var $link = $(link);
        ...
        // changing input to short name text - don't take value from link text
        // ??
    ...
    function _initInsertPopup (delayed) {
      ...
      // input_layer += '<div class="fr-input-line"><input name="href" type="hidden" class="fr-link-attr" placeholder="URL" tabIndex="' + (++tab_idx) + '"></div>';
      input_layer += '<div class="fr-input-line"><input name="href" type="text" class="fr-link-attr" placeholder="URL from dropdown above - do not type here." tabIndex="' + (++tab_idx) + '"></div>';
      ...
      // removes: if (!editor.opts.linkAlwaysBlank) {
    ...
    function insertCallback () {
      ...
      var shortnametext = text_inputs.filter('[name="shortnametext"]').val();
      // add short name to href here
      var href = text_inputs.filter('[name="href"]').val() + shortnametext;
      // added by don
      ...
      insert(href, text, attrs); // , shortnametext
     ...
     */
    function insert (href, text, attrs) { // , shortnametext
      // Stop shortnametext input from creating an attribute
      ...
      */
      ...
      // don change 2016-03-15
      // automatically add class pop_item
      // lastedit
      attrs.class = 'pop_item';
      ...
      if (link) {
        ...
        // don -- takes effect only in (re)edit, not insert
        ...
        if (!$current_image) {
          ...
          // Nothing is selected.
          // don change - disable this, we're hijacking "text" to apply shortname to href, don't want to alter selected text
          if (editor.selection.isCollapsed()) {
            //alert('not changing text - first time around');
            ...
            //editor.html.insert('<a href="' + href + shortnametext +'">' + $.FroalaEditor.START_MARKER + text + $.FroalaEditor.END_MARKER + '</a>');
            editor.selection.restore();
            
            // try adding text to href here
            // href = href + shortnametext;

          }
          else {
            // don change - disable this, we're hijacking "text" to apply shortname to href, don't want to alter selected text
            if (text.length > 0 && text != editor.selection.text()) {
              //alert('not changing text');
              
              editor.selection.remove();
              editor.html.insert('<a href="' + href + '">' + $.FroalaEditor.START_MARKER + text + $.FroalaEditor.END_MARKER + '</a>');
              // editor.html.insert('<a href="' + href + 'second_insert' + shortnametext + '">' + $.FroalaEditor.START_MARKER + text + $.FroalaEditor.END_MARKER + '</a>');
              editor.selection.restore();
